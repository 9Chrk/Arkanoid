...
void checkBitmap(const ALLEGRO_BITMAP* bitmap, const std::string& fileName) {
  if (!bitmap) throw std::runtime_error("Failed to load image: " + fileName);
}

void checkSample(const ALLEGRO_SAMPLE* sample, const std::string& fileName) {
  if (!sample) throw std::runtime_error("Failed to load sound: " + fileName);
}
...
[[gnu::pure]] ALLEGRO_COLOR getColor(const std::string& colorName) {
